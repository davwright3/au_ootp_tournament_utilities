...
def get_rolling_pa(df, df_list=None, window=5):
    ...
    plate_app_df['PA_roll'] = plate_app_df['PA_roll'] / max_pa
...
def get_rolling_woba(df, df_list=None, window=5):
    ...
    woba_df['woba_denominator'] = (
            woba_df['AB_rolling'] + woba_df['BB_rolling'] -
            woba_df['IBB_rolling'] + woba_df['SF_rolling'] +
            woba_df['HP_rolling']
    )

    woba_df['wOBA_rolling'] = (
            woba_df['woba_numerator'] / woba_df['woba_denominator']
    )
...
def get_rolling_avg(df, df_list=None, window=5):
    """Return a dataframe displaying rolling average in matplotlib figure."""
    ...
    avg_df['avg_rolling'] = (
        (avg_df['hits_rolling'] / avg_df['ab_rolling'])
        .round(3)
    )
...
def get_rolling_slg(df, df_list=None, window=5):
    """Return a dataframe displaying rolling SLG in matplotlib figure."""
    ...
    slg_df['slg_rolling'] = (
        (slg_df['tb_rolling'] / slg_df['ab_rolling'])
        .round(3)
    )
...
def get_rolling_obp(df, df_list=None, window=5):
    """Return a dataframe displaying rolling OBP in matplotlib figure."""
    ...
    obp_df['rolling_obp'] = ((
            (obp_df['rolling_hits'] + obp_df['rolling_bb'] +
             obp_df['rolling_hp']) /
            (obp_df['rolling_ab'] + obp_df['rolling_hp'] +
