...
class TreeviewTableFrame(ctk.CTkFrame):
    ...
    def load_dataframe(self, df: pd.DataFrame, min_pa=1):
        ...
        for col in df.columns:
            ...
            if col == "Title":
                self.tree.column(col, anchor="w", width=350, minwidth=100, stretch=True)
            else:
    ...
    def sort_by_column(self, col):
        reverse = not (self.sort_state["col"] == col and self.sort_state["reverse"])
        self.sort_state = {"col": col, "reverse":reverse}

        try:
            sorted_df = self.df.sort_values(by=col, ascending=not reverse, key=lambda x: pd.to_numeric(x, errors='coerce'))
        except:
