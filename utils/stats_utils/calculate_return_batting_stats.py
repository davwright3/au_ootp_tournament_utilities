...
def calculate_obp(hits, bbs, hp, sf, at_bats):
    ...
    return ((hits.sum() + bbs.sum() + hp.sum()) / (at_bats.sum() + bbs.sum() + hp.sum() + sf.sum())).round(3)
...
def calculate_woba(bbs, hp, single, double, triples, hr, at_bats, ibb, sf):
    ...
    return (((.701*bbs.sum()) + (.732*hp.sum()) + (.895*single.sum()) + (1.270*double.sum()) + (1.608*triples.sum()) + (2.072* hr.sum())) / (at_bats.sum() + bbs.sum() - ibb.sum() + sf.sum() + hp.sum())).round(3)
